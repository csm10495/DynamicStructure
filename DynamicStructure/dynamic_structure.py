...
def getStructureType(fieldTuple, buffer, parent=Structure, pack=1):
    '''
...
def getDynamicStructure(fields, buffer=None, pack=1):
    '''
    ...
    for idx, fieldTuple in enumerate(fields):
        BuildStructure = getStructureType(fieldTuple, buffer, BuildStructure, pack=pack)
        
    return BuildStructure().fill(buffer)
